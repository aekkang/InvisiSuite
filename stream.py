...
def rengfunc(hosts=["172.20.10.4"], visual=True):
    ...
    with open('circle_jerk.json', 'r') as infile:
        circle_jerk = json.load(infile)
        while True:
            ### Read streams.
            ...
            if start != -1 and end != -1:
                ...
                if clicked:
                    ...
                    ####
                    more_key = str(jerked) + "more"
                    less_key = str(jerked) + "less"
                    if more_key in circle_jerk:
                        jerk_ratio_more = circle_jerk[more_key]
                    if less_key in circle_jerk:
                        jerk_ratio_less = circle_jerk[less_key]

                    if jerked == 4 or jerked == 5: # SPACE CASE
                        offset = 0
                    elif jerked == 9:
                        offset = 1
                    elif jerked == 7 or jerked == 8:
                        if finger_lengths[jerked] > base_lengths[jerked] * 1.1:
                            offset = 1
                    else:
                        if finger_lengths[jerked] > base_lengths[jerked] * 1.1:
                            offset = 1
